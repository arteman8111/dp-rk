...
function rungekutta(el, dt, t, thet_torch, thet_2) {
    // debugThis(t,665.200)   
    let k1_vx = dt * vx_toch(t, el[0], el[3], el[4], P(t), thet_torch, thet_2);
    let k1_vy = dt * vy_toch(t, el[0], el[3], el[4], P(t), thet_torch, thet_2);
    let k1_x = dt * x_toch(el[1]);
    let k1_y = dt * y_toch(el[2]);
    let k1_m = dt * m_toch(P(t))

    let k2_vx = dt * vx_toch(t + dt / 2, el[0] + k1_m / 2, el[3] + k1_x / 2, el[4] + k1_y / 2, P(t), thet_torch, thet_2);
    let k2_vy = dt * vy_toch(t + dt / 2, el[0] + k1_m / 2, el[3] + k1_x / 2, el[4] + k1_y / 2, P(t), thet_torch, thet_2);
    let k2_x = dt * x_toch(el[1] + k1_vx / 2);
    let k2_y = dt * y_toch(el[2] + k1_vy / 2);
    let k2_m = dt * m_toch(P(t));

    let k3_vx = dt * vx_toch(t + dt / 2, el[0] + k2_m / 2, el[3] + k2_x / 2, el[4] + k2_y / 2, P(t), thet_torch, thet_2);
    let k3_vy = dt * vy_toch(t + dt / 2, el[0] + k2_m / 2, el[3] + k2_x / 2, el[4] + k2_y / 2, P(t), thet_torch, thet_2);
    let k3_x = dt * x_toch(el[1] + k2_vx / 2);
    let k3_y = dt * y_toch(el[2] + k2_vy / 2);
    let k3_m = dt * m_toch(P(t));

    let k4_vx = dt * vx_toch(t + dt, el[0] + k3_m, el[3] + k3_x, el[4] + k3_y, P(t), thet_torch, thet_2);
    let k4_vy = dt * vy_toch(t + dt, el[0] + k3_m, el[3] + k3_x, el[4] + k3_y, P(t), thet_torch, thet_2);
    ...
    el[7] = thet(t, thet_torch, thet_2);
    el[8] = TETA(el[1], el[2], el[3], el[4], el[5], el[6]);
    el[9] = alpha(el[2], el[4]);
    el[10] =fi(el[3], el[4]);
